...
// @ts-ignore
import styles from './/src/components/ListItem.module.css';

function ListItem({ icon, text, active, badge }) {
  const renderIcon = () => {
    switch (icon) {
      case 'friends':
        return (
  ...
  return (
    <div
      className={`${styles.listItem} ${active ? styles.active : ''} ${
        icon === 'shop' ? styles.shop : ''
      }`}
    >
      {renderIcon()}
      <p>{text}</p>
      {badge && (
        <div className={badge === 'NEW' ? styles.new : styles.buy}>{badge}</div>
      )}
    </div>
  );
}
